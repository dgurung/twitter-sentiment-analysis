...
class ConnectionHandler {
 public:
  ...
  ConnectionHandler(ConnectionHandler&& other) {
    ...
    other.m_curl_handler =
        nullptr;  // Release the curl handler of source object.
  }
...
void ConnectionHandler<Derived_t>::setup_curl_options() {
  ...
  // - User agent name
  m_status =
      curl_easy_setopt(m_curl_handler, CURLOPT_USERAGENT, "libcurl-agent/1.0");
  if (m_status != CURLE_OK) {
    throw std::runtime_error(std::string{curl_easy_strerror(m_status)});
  }
...
void ConnectionHandler<Derived_t>::setup_callback() {
  ...
  // - Write data
  m_status =
      curl_easy_setopt(m_curl_handler, CURLOPT_WRITEDATA, (void*)&m_queue);
  if (m_status != CURLE_OK) {
    throw std::runtime_error(std::string{curl_easy_strerror(m_status)});
  }
}
...
void ConnectionHandler<Derived_t>::post_request(const std::string& data) {
  ...
  m_status =
      curl_easy_setopt(m_curl_handler, CURLOPT_POSTFIELDSIZE, data.length());
  if (m_status != CURLE_OK) {
    throw std::runtime_error(std::string{curl_easy_strerror(m_status)});
  }
